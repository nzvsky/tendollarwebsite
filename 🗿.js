...
const intros = [
    {name: $('#caption').text(), path: "assets/intros/1.wav" },
    {name: "how you gonna talk behind my back when you deadass built like a", path: "assets/intros/2.wav" },
    {name: "white people be like", path: "assets/intros/3.wav" }
]

const actions = [
    { shortcut: "t", action: "Set tempo", amount: true, name: "speed", image: "assets/icons/oem/action_speed.png", default: 300, set: [10, 999999], add: [-10000, 10000], multiply: [0.01, 999999, 0.1], divide: [0.1, 999999, 0.1] },
    { shortcut: "v", action: "Set volume", amount: true, name: "volume", image: "assets/icons/oem/action_volume.png", default: 100, set: [0, 999999, 1, "%"], add: [-999999, 999999, 1, "%"], multiply: [0.01, 999999, 0.1], divide: [0.1, 999999, 0.1] },
    { shortcut: "p", action: "Pause for duration", amount: true, name: "stop", image: "assets/icons/oem/action_stop.png", default: 4, set: [0, 999999] },
    { shortcut: "m", action: "Transpose", amount: true, name: "transpose", image: "assets/icons/oem/action_transpose.png", default: 1, set: [-999999, 999999], add: [-999999, 999999] },
 
    { shortcut: "l", action: "Loop", amount: true, name: "loopmany", image: "assets/icons/oem/action_loopmany.png", default: 4, set: [1, 999999] },
    { shortcut: "r", action: "Loop once", name: "loop", image: "assets/icons/oem/action_loop.png" },
    { shortcut: "s", action: "Set loop start point", name: "looptarget", image: "assets/icons/oem/action_looptarget.png" },
    { shortcut: "c", action: "Combine sounds", name: "combine", image: "assets/icons/oem/action_combine.png" },
    
    { shortcut: "g", action: "Go to target", isTarget: true, name: "jump", image: "assets/icons/oem/action_jump.png", set: [1, 999999] },
    { shortcut: "a", action: "Target", isTarget: true, name: "target", image: "assets/icons/oem/action_target.png", set: [1, 999999] },
    { shortcut: "x", action: "Stop all sounds", name: "cut", stopSounds: true, image: "assets/icons/oem/action_cut.png" },
    { shortcut: "o", action: "Set start position", name: "startpos", image: "assets/icons/oem/action_startpos.png" },

    { shortcut: "d", action: "Add divider", name: "divider", image: "assets/icons/oem/action_divider.png" },
    { shortcut: "f", action: "Flash screen", name: "flash", image: "assets/icons/oem/action_flash.png" },
    { shortcut: "u", action: "Pulse screen", amount: true, twoValues: [[0, 1000], [0.1, 128]], scroll: [0, 1], default: [1, 2], name: "pulse", image: "assets/icons/oem/action_pulse.png" },
    { shortcut: "b", action: "Set background color", amount: true, colorMode: true, twoValues: [["color"], [0, 128]], scroll: [1, 0.25], default: ["X", 1], name: "bg", image: "assets/icons/oem/action_bg.png" }
]
...
fetch("./sounds.json").then(x => x.json()).then(list => {
    ...
    list.forEach(x => {
        let imageLink = (!x.emoji && x.id.match(/[a-z0-9]/i)) ? `assets/icons/sounds/${x.img || x.id}.png` : `https://cdnjs.cloudflare.com/ajax/libs/twemoji/14.0.2/svg/${(x.emoji || x.id).codePointAt(0).toString(16)}.svg`
        $('#icons').append(`<div class="sound" soundName="${x.name}" soundOrigin="${x.source || ""}" sound="assets/sounds/${x.id}.wav" str="${x.id}" ${(x.tags || []).map(x => "tag_" + x).join(" ")}><img alt="${x.name}" src="${imageLink}"></div>`)
        lastGroup = x.group
    })
